...
# License policy - Apache-2.0 compatible licenses
...
REVIEW_REQUIRED_LICENSES: Set[str] = {
    ...
    "LGPL-2.1",
    "LGPL-3.0",
    "EPL-1.0",  # Older Eclipse Public License
}
...
def load_license_overrides() -> Dict[str, str]:
    ...
    try:
        with open(overrides_path, 'r') as f:
            data = json.load(f)
            # Convert to dict: "name@version" -> "license"
            return {
...
def get_manual_license(name: str, version: str, overrides: Dict[str, str]) -> Optional[str]:
...
def check_license_policy(sbom_path: str) -> Tuple[bool, List[str], List[str], List[str], List[str]]:
    """
    Check license policy compliance.

    Returns:
        (passed, disallowed_components, review_required_components, unknown_components, manual_overrides)
    """
    try:
    ...
    for component in components:
        ...
        if manual_license and not licenses:
            ...
            manual_overrides.append(f"{name} {version} - Manually verified: {manual_license}")
    ...
    return passed, disallowed_components, review_required_components, unknown_components, manual_overrides
...
def main():
    ...
    passed, disallowed, review_required, unknown, manual_overrides = check_license_policy(sbom_path)

    print("=" * 80)
    print("License Policy Check - videostream-rs")
    print("=" * 80)
    print()

    if manual_overrides:
        print("MANUALLY VERIFIED LICENSES:")
        print("-" * 80)
        for component in manual_overrides:
            print(f"  ✓ {component}")
        print()
